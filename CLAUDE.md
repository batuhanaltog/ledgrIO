...
| **4. Transactions + Categories** | ✅ **Tamamlandı** | `apps/categories/` (7 system cats, unlimited hierarchy, system/user-owned, soft delete) + `apps/transactions/` (income/expense, FX snapshot at write time, filters: type/category/currency/date/amount/search, summary: totals + running balance by day/week/month). ~45 yeni test, %93 coverage. |
